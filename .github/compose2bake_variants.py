...
def theme_hcl(servicename: str, servicedef: Dict[str, Any]) -> Tuple[Sequence[str], str]:
    """Matrix HCL for themed images"""
    hcl_targets = ""
    img_wotheme = servicedef["image"].replace("-default-", "-${vite_theme}-")
    ...
    hcl_targets += f"""
target "{servicename}" {{
    name = "{servicename}-${{vite_theme}}"
    matrix = {{
        vite_theme = [{", ".join(f'"{vite_theme}"' for vite_theme in VITE_THEMES)}]
    }}
...
"""
    ...
    if "args" in servicedef["build"]:
        ...
        for argname, argval in servicedef["build"]["args"].items():
            if argname == "VITE_THEME":
                hcl_targets += """        VITE_THEME: "${vite_theme}"\n"""
            else:
                hcl_targets += f"""        {argname}: "{argval}"\n"""
        hcl_targets += "    }\n"

    hcl_targets += "}"
    return [servicename], hcl_targets


def service_hcl(servicename: str, servicedef: Dict[str, Any]) -> Tuple[Sequence[str], str]:
    """Make the HCL"""
    if "args" in servicedef["build"] and "VITE_THEME" in servicedef["build"]["args"]:
        return theme_hcl(servicename, servicedef)
...
async def main() -> None:
    ...
    for servicename, servicedef in parsed["services"].items():
        if "build" not in servicedef:
            continue
        if servicename in ("takreten", "takplug", "takmsg", "takconfig", "takapi"):
            continue
